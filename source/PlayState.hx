...
class PlayState extends MusicBeatState
{
	...
	//public static var peggyBeaten:Bool = false;
	//public static var vileBeaten:Bool = false;
	...
	override public function create()
	{
		...
		switch(Paths.formatToSongPath(SONG.song))
		{
			...
			case 'batness' | "pigrush" | "bloodbath" | "pickle-rave" | "memories":
				GameOverSubstate.characterName = "geeflover69-dead";
				GameOverSubstate.deathSoundName = 'geeflover69Dead';
		}
		...
		timeTxt.setFormat(Paths.font("gothambold.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("msmincho.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("msmincho.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if (Paths.formatToSongPath(SONG.song) == 'peggy')
				{
					Achievements.unlockAchievement("cocksucker");
				}
				#end
	...
	{
		/*if(curSong == 'vile'){
			ClientPrefs.vileBeaten = true;
					ClientPrefs.saveSettings();
									 MusicBeatState.switchState(new StoryMenuState());    
			FlxG.sound.playMusic(Paths.music('freakyMenu'));
		}
		else if(curSong == 'peggy'){
			ClientPrefs.peggyBeaten = true;
					ClientPrefs.saveSettings();
									 MusicBeatState.switchState(new StoryMenuState());    
			FlxG.sound.playMusic(Paths.music('freakyMenu'));
		}
		...
		if(Paths.formatToSongPath(SONG.song) == 'peggy' && !practiceMode) 
			{
				ClientPrefs.peggyBeaten = true;
				FlxG.save.data.peggyBeaten = ClientPrefs.peggyBeaten;
				FlxG.save.flush();

			}
			if(Paths.formatToSongPath(SONG.song) == 'vile' && !practiceMode) 
			{
				ClientPrefs.vileBeaten = true;
				FlxG.save.data.vileBeaten = ClientPrefs.vileBeaten;
				FlxG.save.flush();
			}
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['chapter1miss', 'chapter1nomiss', 'peggymybeloved', 'cocksucker', 'picklecheck', 'memory']);
					...
					switch(achievementName)
					{
						case 'chapter1nomiss':
							if(isStoryMode && campaignMisses + songMisses < 1 && storyPlaylist.length <= 1 && !usedPractice)
							{
								...
								{
									case 'ch1Week':
										if(achievementName == 'chapter1nomiss') unlock = true;
								}
							}
						case 'chapter1miss':
							if(isStoryMode && storyPlaylist.length <= 1 && !usedPractice)
							{
								...
								{
									case 'ch1Week':
										if(achievementName == 'chapter1miss') unlock = true;
								}
							}
						case 'memory':
							if(Paths.formatToSongPath(SONG.song) == "memories" && !practiceMode) unlock = true;
						case 'picklecheck':
							if(Paths.formatToSongPath(SONG.song) == "pickle-rave" && !practiceMode) unlock = true;
						case 'peggymybeloved':
							if(Paths.formatToSongPath(SONG.song) == "peggy" && !practiceMode) unlock = true;
					}
